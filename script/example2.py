...
def generate_symmetric_positive_noise(N, bmin, bmax):
    ...
    scale_factor = 0.2 * (bmax - bmin) / max_val  # Scaling factor
    noise *= scale_factor 
...
def generate_latex_table_noise():
    ...
    n_values = [n for n in range(2, 16)]
    ...
    with open("../rslt/example2noise.tex", "w") as f:
        f.write(latex_table)
    ...
    plt.scatter(N, np.log(mc), color='blue', label=r'$L_{\text{Haf}^2}^\text{MC}$')
    plt.plot(N, N**2*np.log(1.3), color='red', label=r'${1.3}^{N^2}$')
    plt.xlabel('N')
    plt.ylabel('log(Values)')
    plt.legend()
    plt.grid(True)
    plt.savefig("../rslt/gc-hafsq-mc-log-n.png", bbox_inches="tight")

    N = np.array(n_values)
    gbs = np.array(gbs)
    plt.figure(figsize=(8, 6))
    plt.scatter(np.log(N), np.log(gbs), color='blue', label=r'$U_{\text{Haf}^2}^\text{GBS-I}$')
    plt.plot(np.log(N), np.log(cubic_function(N, 1)), color='red', label=r'$N^3$')
    plt.xlabel('log(N)')
    plt.ylabel('log(Values)')
    plt.legend()
    plt.grid(True)
    plt.savefig("../rslt/gc-hafsq-gbs-log-n.png", bbox_inches="tight")
