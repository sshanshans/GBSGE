...
with open(f"{path_to_folder}/{exp_id}.pkl", "rb") as f:
    T = pickle.load(f)
print('master', T.B.compute_master_bmat_det())
print('gt', T.gt)
...
for which_rslt in ['val_est', 'mul_err', 'add_err']:
    path_to_estimates=f"{path_to_folder}/estimates/GBSI"
    ...
    plotter.plot_threads_single_method(est_gbsi, color='red', label="GBS-I")
    ...
    plotter.set_ylim(crete_ylim(which_rslt, T.gt, 0.1))
    plotter.set_tick_labels(total_sample_used, step_size)
    plotter.save_plot(f'{path_to_folder}/figures/{which_rslt}_large.png')
