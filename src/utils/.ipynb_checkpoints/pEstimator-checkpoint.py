        ...
        for i in range(num_tasks):
            ...
            for n, x in enumerate(xs):
                ...
                if (current_size + 1) % step_size == 0:
                    current_estimate = np.sum([aI * np.sqrt(d_inv * ifac(I) * query_single_I_from_count_dict(count_dict, I)/(current_size + 1)) for I, aI in T.enumerate_all_aI()]) + T.a0 * np.sqrt(d_inv * count_a0/(current_size + 1))
