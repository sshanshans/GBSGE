...
class GBSLI(ProcessingMethod):
    '''
    This is the class of GBS-I estimators that first compute the optimal t and then sample according to the new parameters
    '''
    def __init__(self):
        self.name = 'GBSLI'
    ...
    def process(self, T, num_samples):
        es = []
        xs = generate_sample_at_level_k(T, num_samples)
        for x in xs:
            es.append(evaluate_sample(T, x))
        return es
